...
to go
  ...
  if (member? decision-scenario ["deciding-as-usual" "nash-equilibrium"]) [
    ask turtles [
      ifelse (signal-direction = "B" and signal-strength >= threshold)
      [ set imposer? true ]
      [ set imposer? false ]
    ]
  ]
 ...
 ;; SCENARIO: deference-leader
  ...
  if (decision-scenario = "deference-vote") [
    ...
    ; For visualization, we mark the turtles who voted for B as "imposers", even if the group choses not to impose.
    ask turtles [ ifelse signal-direction = "B" [ set imposer? true ] [set imposer? false] ]
  ]

  ;; SCENARIO: deference-weighted-vote
  if (decision-scenario = "deference-weighted-vote") [
    ; In this scenario, the weight of an agent's vote is determined by their signal strength. We sum the "conviction" for each side.

    ; To normalize, we subtract 50 from the strength so a 51% signal has a weight of 1, and a 100% signal has a weight of 50.
    let conviction-for-b sum [signal-strength - 50] of turtles with [signal-direction = "B"]
    let conviction-for-a sum [signal-strength - 50] of turtles with [signal-direction = "A"]

    ; The group imposes if and only if the total conviction for B is greater. Ties default to A (the status quo).
    set imposed? (conviction-for-b > conviction-for-a)
  ...
  if (decision-scenario = "limited-imposers") [
    ask turtles [ set imposer? false ] ; Everyone defaults to abstaining.
    ask n-of round (percent-active-imposers * group-size / 100) turtles [ ; A subset is chosen to be active.
      if (signal-direction = "B" and signal-strength >= threshold) [
        set imposer? true
      ]
    ]
  ...
  ; This logic works for all scenarios except the vote, which sets 'imposed?' directly.
   if (member? decision-scenario ["deciding-as-usual" "nash-equilibrium" "deference-leader" "limited-imposers"]) [
...
; Step 6: Update global trackers for the round.
  ifelse correct?
  [
    set rounds-won rounds-won + 1
    ifelse imposed?
      [ set good-impositions good-impositions + 1 ]
      [ set good-abstentions good-abstentions + 1 ]
  ]
  [
    ifelse imposed?
      [ set bad-impositions bad-impositions + 1 ]
      [ set missed-opportunities missed-opportunities + 1 ]
  ]

  ; Step 7: Update patch colors based on the final outcome.
...
decision-scenario
decision-scenario
"deciding-as-usual" "nash-equilibrium" "deference-leader" "deference-vote" "deference-weighted-vote" "limited-imposers"
1
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks rounds-won * 100 / max list 1 ticks"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks bad-impositions * 100 / max list 1 ticks"
...
This model is also available at https://github.com/kvnalb/unilateralist
...
<experiments>
  ...
  <experiment name="Comprehensive Sweep" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>repeat 100 [go]</go>
    <exitCondition>ticks = 100</exitCondition>
    <metric>rounds-won</metric>
    <metric>bad-impositions</metric>
    <enumeratedValueSet variable="decision-scenario">
      <value value="&quot;deciding-as-usual&quot;"/>
      <value value="&quot;nash-equilibrium&quot;"/>
      <value value="&quot;deference-leader&quot;"/>
      <value value="&quot;deference-vote&quot;"/>
      <value value="&quot;deference-weighted-vote&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="group-size" first="2" step="1" last="100"/>
    <enumeratedValueSet variable="reward-amount">
      <value value="&quot;1.00&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Limited Imposers" repetitions="1000" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>repeat 100 [go]</go>
    <exitCondition>ticks = 100</exitCondition>
    <metric>rounds-won</metric>
    <metric>bad-impositions</metric>
    <enumeratedValueSet variable="decision-scenario">
      <value value="&quot;limited-imposers&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="group-size" first="10" step="10" last="100"/>
    <steppedValueSet variable="percent-active-imposers" first="10" step="10" last="100"/>
    <enumeratedValueSet variable="reward-amount">
      <value value="&quot;1.00&quot;"/>
    </enumeratedValueSet>
  </experiment>
